...
int main (int argc, char *argv[])
{
  ...
  {
    sprintf (buf, "can't find filesize");
    perror(buf);
    exit(errno);
  }
   ...
   */
  if ( write( fdout, buf, 1 ) < 0 )
  {
    sprintf (buf, "can't write\n");
    perror(buf);
    exit(errno);
  }
   ...
   */
  src = mmap( NULL, length, PROT_READ, MAP_SHARED, fdin, 0 );

  /* 
   * 5. mmap the output file 
   */
  dst = mmap( NULL, length, PROT_READ | PROT_WRITE, MAP_SHARED, fdout, 0 );
